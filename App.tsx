...
const ResumeDocument = ({ data, preset, alignment }: { data: ResumeData, preset: string, alignment: 'left' | 'center' | 'right' }) => {
        ...
        return (
          <View style={styles.section} key={id} wrap={false}>
            ...
            <View style={styles.metricsRow}>
              {Object.entries(data.highlights).map(([key, val]) => (
  ...
  return (
    <Document title={`${data.personalInfo.fullName} Resume`}>
...
const App: React.FC = () => {
  ...
  const [currentResumeId, setCurrentResumeId] = useState<string>(DEFAULT_RESUME.id);
    ...
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900">
        ...
        {status === 'fetching' ? (
            ...
            <>
                ...
                {activeTab === 'Build' && (
                  <Editor 
                    data={currentResume} 
                    onChange={(fields) => updateResume(currentResume.id, fields)} 
                ...
                {activeTab === 'Preview' && (
                  <Preview 
                    data={currentResume} 
                    onGeneratePDF={startExportSequence}
                    onUpdate={(fields) => updateResume(currentResume.id, fields)}
                    selectedPreset={currentResume.selectedPreset || 'bold'}
                    onPresetChange={(p) => updateResume(currentResume.id, { selectedPreset: p })}
                  />
                )}
                {activeTab === 'Admin' && (
                  <Admin 
      ...
      {showConfirm && (
        <div className="fixed inset-0 z-[20000] bg-slate-900/40 backdrop-blur-md flex items-center justify-center p-4">
          <div className="bg-white rounded-[2.5rem] p-10 max-md w-full shadow-2xl animate-fade-in border border-slate-100">
